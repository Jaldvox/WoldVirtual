...
class BlockchainSimple:
    ...
    def _initialize_blockchain(self):
        ...
        genesis_block = self.blockchain_manager('create_block', 0, [], '0' * 64)
        if genesis_block:
            self.chain.append(genesis_block)

    def blockchain_manager(self, operation: str, *args) -> Any:
        """
        Unified blockchain operation manager with error handling
        ...
        Returns:
            Operation result or None if invalid operation
        """
        try:
            if operation == 'create_block':
                if len(args) != 3:
                    raise ValueError("create_block requires 3 arguments: index, transactions, previous_hash")
                ...
                if len(args) != 3:
                    raise ValueError("transaction requires 3 arguments: sender, recipient, amount")
                ...
                if not args or not isinstance(args[0], str):
                    raise ValueError("mining requires miner address")
                ...
                if not args or not isinstance(args[0], str):
                    raise ValueError("balance requires address")
                ...
                if not args:
                    raise ValueError("hash requires data to hash")
                return hashlib.sha256(json.dumps(args[0], sort_keys=True).encode()).hexdigest()

            return None
        except Exception as e:
            print(f"Error in blockchain operation '{operation}': {str(e)}")
            return None
