...
class UserManager:
    ...
    def register_user(self, username, password):
        ...
        self.users[username] = {
            ...
            'resources': ResourceManager(cpu_limit=10, bandwidth_limit=20)
        }
    ...
    def handle_action(self, username, action):
        if username not in self.users:
            return False, "User not found"
            
        if action == "explore":
            return True, f"Welcome {username} to the exploration environment"
        elif action == "trade":
            return True, f"Processing trade for {username}"
        else:
            return False, "Unrecognized action"

# Resource management system
class ResourceManager:
    def __init__(self, cpu_limit, bandwidth_limit):
        self.cpu_limit = cpu_limit
        self.bandwidth_limit = bandwidth_limit
        self.cpu_usage = 0
        self.bandwidth_usage = 0
        
    def allocate_resources(self, cpu_request, bandwidth_request):
        if cpu_request <= self.cpu_limit and bandwidth_request <= self.bandwidth_limit:
            self.cpu_usage = cpu_request
            self.bandwidth_usage = bandwidth_request
            return True
        return False
...
class SystemMonitor:
    ...
    def get_system_status(self):
        ...
        for username, user_data in self.user_manager.users.items():
            status[username] = user_data['resources'].get_usage()
        return status

# Initialize core components
user_manager = UserManager()
system_monitor = SystemMonitor(user_manager)

# Example usage
success, message = user_manager.register_user("user1", "secure_password")
print(message)

if user_manager.verify_credentials("user1", "secure_password"):
    success, message = user_manager.handle_action("user1", "explore")
    print(message)
